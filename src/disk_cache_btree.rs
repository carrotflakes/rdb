...
use crate::{
    btree::{BTree, BTreeNode, NodeRef},
    disk::{Page, PAGE_SIZE},
    disk_cache::{DiskCache, PageContainer, PageOps, PageRef},
};
...
impl BTreeNode<Key, Value> for Page {
    ...
    fn set_parent(&mut self, _: &Self::Meta, i: usize) {
        self[1..1 + 4].copy_from_slice(&(i as u32).to_le_bytes());
    }
        ...
        if self.is_leaf(meta) {
            ...
            let (pivot_index, pivot_key) = match meta {
                ...
                Meta {
                    ...
                    let offset = LEAF_HEADER_SIZE + key_interval * (pivot_index - 1);
                    let key_index_end = parse_u16(&self[offset..offset + INDEX_SIZE]) as usize;
                ...
                Meta {
                    ...
                    let get_key = |this: &Self, index: usize| {
                        let key_offset = parse_u16(
                    ...
                    let (pivot_index, pivot_key) = {
                        let mut index = size / 2;
                        let key = get_key(self, index);
                        for i in (0..index).rev() {
                            let k = get_key(self, i);
                            if self[key.clone()] != self[k] {
                                index = i + 1;
                                break;
                            }
                    ...
                    let offset = LEAF_HEADER_SIZE + key_interval * (pivot_index - 1);
                    let key_index_end = parse_u16(&self[offset..offset + INDEX_SIZE]) as usize;
                    let to_move_size = key_index_end - last_key_index;
                    new_page[PAGE_SIZE as usize - to_move_size..PAGE_SIZE as usize]
                        .copy_from_slice(&self[last_key_index..key_index_end]);
            ...
            let value_size = 4;
            let (pivot_index, pivot_key) = match meta.key_size {
                // pivot_indexというよりはsizeを表している
                Some(key_size) => {
                    let (pivot_index, pivot_key) = {
                        let mut index = (size + 1) / 2;
                        let key =
                            self.slice(INTERNAL_HEADER_SIZE + key_size * (index - 1), key_size);
    ...
    fn get_child(&self, meta: &Self::Meta, key: &Key) -> usize {
        ...
        match meta.key_size {
            Some(key_size) => {
                ...
                let offset = PAGE_SIZE as usize - value_size * index;
    ...
    fn init_as_root_internal(&mut self, meta: &Self::Meta, key: &Key, i1: usize, i2: usize) {
        self[0] = 0;
        self.set_parent(meta, 0);
        self.set_size(2);
        self.set_next(meta, 0);
    ...
    fn insert_value(&mut self, meta: &Self::Meta, key: &Key, value: &Value) -> bool {
        ...
        match meta {
            Meta {
                ...
                debug_assert_eq!(*key_size, key.len());
                debug_assert_eq!(*value_size, value.len());
            ...
            Meta {
                ...
                for i in 0..size {
                    let offset =
                        parse_u16(self.slice(LEAF_HEADER_SIZE + INDEX_SIZE * i, INDEX_SIZE))
                            as usize;
                    let k = &self[offset..insert_offset - value_size];
                    if key.as_slice() < k {
                        insert_index = i;
                        break;
                    }
                ...
                self.slice_mut(LEAF_HEADER_SIZE + INDEX_SIZE * insert_index, INDEX_SIZE)
                    .copy_from_slice(
                        &((insert_offset - value_size - key.len()) as u16).to_le_bytes(),
                    );
                ...
                self.slice_mut(insert_offset - value_size - key.len(), key.len())
                    .copy_from_slice(key);
                self[insert_offset - value_size..insert_offset].copy_from_slice(value);
            }
            Meta {
                key_size: None,
                value_size: None,
                ...
                for i in (insert_index * 2..size * 2).rev() {
                    ...
                    let key_index = parse_u16(slice_from);
                    let slice_to =
                        self.slice_mut(LEAF_HEADER_SIZE + INDEX_SIZE * (i + 2), INDEX_SIZE);
                    slice_to.copy_from_slice(
    ...
    fn set_next(&mut self, _: &Self::Meta, i: usize) {
        self[1 + 4 + 2..1 + 4 + 2 + 4].copy_from_slice(&(i as u32).to_le_bytes());
    }

    fn find_cursor(&self, meta: &Self::Meta, key: &Key) -> (usize, bool) {
        let size = self.size(meta);
        match meta {
            Meta {
                key_size: Some(key_size),
    ...
    fn cursor_get(&self, meta: &Self::Meta, cursor: usize) -> Option<(Key, Value)> {
        match meta {
            Meta {
                ...
                let key = self.slice(key_index, *key_size).to_vec();
                let value_index = PAGE_SIZE as usize - value_size * (cursor + 1);
                let value = self.slice(value_index, *value_size).to_vec();
...
impl NodeRef<Page> for PageRef {
    type R = PageContainer;
    type W = PageContainer;

    fn read(&self) -> std::sync::RwLockReadGuard<Self::R> {
        self.read()
    }

    fn write(&self) -> std::sync::RwLockWriteGuard<Self::W> {
        self.write()
    }
}

impl BTree<Key, Value> for DiskCache {
    type Node = Page;
    type NodeRef = PageRef;
    ...
    fn add_root_node(&mut self) -> usize {
        ...
        page.set_parent(&meta, 0);
        page.set_size(0);
        page.set_next(&meta, 0);

        self.add_page(page)
    }

    fn get_node(&self, node_i: usize) -> PageRef {
        self.get(node_i)
    }
    ...
    fn swap(&mut self, node_i: usize, mut node: Self::Node) -> Self::Node {
        std::mem::swap(&mut self.get(node_i).write().deref_mut().page, &mut node);
        node
    }
}
...
fn test() {
    ...
    let node_i = dbg!(disk_cache.add_root_node());
    dbg!(disk_cache.find(&meta, node_i, &vec![0, 1, 2, 0, 0, 0, 0, 0]));
    dbg!(disk_cache.insert(
        &meta,
        node_i,
        &vec![0, 1, 2, 0, 0, 0, 0, 0],
        &vec![3, 1, 2, 0, 0]
    ...
    .unwrap();
    let cursor = disk_cache.find(&meta, node_i, &vec![0, 1, 2, 0, 0, 0, 0, 0]);
    dbg!(disk_cache.cursor_get(&meta, &cursor.0));
}
