        ...
        if self.is_leaf(meta) {
            ...
            match meta {
                Meta {
                    ...
                    // move values
                    new_page
                        [PAGE_SIZE as usize - value_size * (size - to_size)..PAGE_SIZE as usize]
                        .copy_from_slice(
            ...
            match meta.key_size {
                Some(key_size) => {
                    let pivot_key = self
                        .slice(INTERNAL_HEADER_SIZE + key_size * (to_size - 1), key_size)
                        .to_vec();

                    // move keys
                    new_page[INTERNAL_HEADER_SIZE
                        ..INTERNAL_HEADER_SIZE + key_size * (size - 1 - (to_size))]
                        .copy_from_slice(
                            &self[INTERNAL_HEADER_SIZE + key_size * (to_size)
                ...
                None => {
                    // untested yet
                    let key_interval = INDEX_SIZE;
                    let offset = INTERNAL_HEADER_SIZE + key_interval * (to_size - 1);
                    let key_index_start = parse_u16(&self[offset..offset + key_interval]) as usize;
                    let key_index_end = parse_u16(&self[offset - key_interval..offset]) as usize;
                    let pivot_key = self[key_index_start..key_index_end - value_size].to_vec();
                    ...
                    // move key_indexs
                    new_page[INTERNAL_HEADER_SIZE
                        ..INTERNAL_HEADER_SIZE + key_interval * (size - 1 - to_size)]
                        .copy_from_slice(
                            &self[INTERNAL_HEADER_SIZE + key_interval * to_size
